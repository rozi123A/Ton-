...
import {
  ...
  Award, TrendingUp, Shield, Plus, PlusCircle, Play, Image as ImageIcon, X, Eye, MessageCircle, Trash2
} from "lucide-react";
...
export default function Profile() {
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-[#080d1a] text-white" dir="rtl">
      ...
      <div className="container mx-auto flex max-w-lg flex-col space-y-3 px-3 py-4">
        ...
        {/* ── Hero card ─────────────────────────────────────────────────── */}
        <section className="relative order-1 overflow-hidden rounded-3xl border border-purple-300/15 bg-[#141e32]/95 shadow-xl shadow-purple-950/30">
          <div className="relative h-24 overflow-hidden bg-gradient-to-l from-indigo-700 via-purple-700 to-fuchsia-500">
            <div className="absolute -left-10 -top-20 h-48 w-48 rounded-full border-[24px] border-white/10" />
            <div className="absolute -right-16 top-0 h-32 w-32 rounded-full bg-fuchsia-300/20 blur-2xl" />
            <div className="absolute inset-x-0 bottom-0 h-16 bg-gradient-to-t from-[#141e32] to-transparent" />
            {isOwnProfile && u?.isPremium && (
              <div className="absolute left-3 top-3 flex items-center gap-1 rounded-full border border-yellow-200/40 bg-yellow-300/90 px-2.5 py-1 text-[10px] font-black text-slate-900 shadow-lg shadow-yellow-950/20">
                <Crown className="h-3 w-3" /> عضو VIP
              </div>
            )}
          </div>

          <div className="relative px-4 pb-4 sm:px-5">
            <div className="mb-3 -mt-12 flex items-end gap-3">
              <div className="relative group">
                <div className="relative h-20 w-20 flex-shrink-0 overflow-hidden rounded-3xl border-4 border-[#141e32] bg-slate-950 shadow-xl shadow-black/40 ring-1 ring-white/15">
                  {avatar ? (
                ...
                {isOwnProfile && (
                  <button 
                    ...
                    className="absolute -bottom-1 -right-1 flex h-7 w-7 items-center justify-center rounded-xl border-2 border-[#141e32] bg-gradient-to-br from-fuchsia-500 to-purple-600 shadow-lg shadow-purple-950/40 transition-transform hover:scale-110"
                  >
              ...
              <div className="flex-1 pb-1">
                 <p className="text-base font-black leading-tight text-white">{name || "مستخدم"}</p>
             ...
             <div className="grid grid-cols-3 gap-2">
               <div className="rounded-xl border border-yellow-300/10 bg-yellow-300/[0.06] p-2 text-center">
                 <div className="mb-0.5 flex items-center justify-center gap-1">
                   <Star className="h-3 w-3 text-yellow-400" />
                   <span className={`text-base font-bold text-yellow-400 ${!isOwnProfile ? 'blur-[4px] select-none' : ''}`}>
                    {isOwnProfile ? (walletQuery.data?.wallet ?? 0) : '—'}
                  </span>
                </div>
                <p className="text-white/50 text-xs">نجوم</p>
              </div>
               <div className="rounded-xl border border-purple-300/10 bg-purple-300/[0.06] p-2 text-center">
                 <div className="mb-0.5 flex items-center justify-center gap-1">
                   <Zap className="h-3 w-3 text-purple-400" />
                   <span className={`text-base font-bold text-purple-400 ${!isOwnProfile ? 'blur-[4px] select-none' : ''}`}>
                    {isOwnProfile ? (balanceQuery.data?.credits ?? 0) : '—'}
                  </span>
                </div>
                <p className="text-white/50 text-xs">نقاط</p>
              </div>
               <div className="rounded-xl border border-emerald-300/10 bg-emerald-300/[0.06] p-2 text-center">
                 <div className="mb-0.5 flex items-center justify-center gap-1">
                   <Award className="h-3 w-3 text-green-400" />
                   <span className="text-base font-bold text-green-400">{isOwnProfile ? completionPct : '??'}%</span>
                </div>
            ...
            {!isOwnProfile && (
               <div className="mt-2 rounded-xl border border-purple-400/20 bg-purple-500/10 px-3 py-1.5 text-center">
        ...
        {(isOwnProfile || (myStoriesQuery.data && myStoriesQuery.data.length > 0)) && (
          <section className="order-6 relative overflow-hidden rounded-2xl border border-fuchsia-300/15 bg-gradient-to-b from-[#18243b] to-[#10192b] p-3 shadow-xl shadow-purple-950/25">
            <div className="pointer-events-none absolute -left-16 -top-20 h-36 w-36 rounded-full bg-fuchsia-500/15 blur-3xl" />
            <div className="relative mb-3 flex items-center justify-between">
              <h2 className="flex items-center gap-2 text-sm font-black text-white">
                <span className="flex h-7 w-7 items-center justify-center rounded-xl bg-gradient-to-br from-fuchsia-500/80 to-purple-600/80 shadow-lg shadow-purple-950/30 ring-1 ring-white/20">
              ...
              {isOwnProfile && (
                <button
                  onClick={() => setShowStoryUpload(true)}
                  className="flex h-7 w-7 items-center justify-center rounded-xl border border-fuchsia-200/25 bg-fuchsia-500/20 text-fuchsia-100 transition hover:bg-fuchsia-500/35"
            ...
            <div className="relative flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {isOwnProfile && (
                <button
                  onClick={() => setShowStoryUpload(true)}
                  className="flex h-28 w-[76px] flex-shrink-0 flex-col items-center justify-center gap-1.5 rounded-xl border border-dashed border-fuchsia-300/35 bg-fuchsia-500/[0.08] text-fuchsia-100 transition hover:border-fuchsia-200/60 hover:bg-fuchsia-500/15"
                >
                  <span className="flex h-8 w-8 items-center justify-center rounded-full bg-fuchsia-500/20 ring-1 ring-fuchsia-200/20">
                    <Plus className="h-4 w-4" />
                  </span>
                  <span className="text-[10px] font-bold">إضافة قصة</span>
                </button>
              )}
              {(myStoriesQuery.data ?? []).map((story: any, index: number) => (
                <div
                  ...
                  className="group relative h-28 w-[76px] flex-shrink-0 cursor-pointer overflow-hidden rounded-xl border border-white/10 bg-slate-950 shadow-md shadow-black/20 transition duration-300 hover:-translate-y-0.5 hover:border-fuchsia-300/40"
                >
                  ...
                  <div className="absolute inset-0 flex flex-col justify-end bg-gradient-to-t from-black/80 via-transparent to-transparent p-1.5">
                    <div className="flex items-center justify-between gap-1">
                      {isOwnProfile ? (
                        <>
                          <div className="flex gap-1 text-[9px] text-white">
                            <span className="flex items-center gap-0.5 rounded-full bg-black/55 px-1.5 py-0.5">
                              <Eye className="h-2.5 w-2.5" /> {story.viewCount || 0}
                            </span>
                            <span className="flex items-center gap-0.5 rounded-full bg-black/55 px-1.5 py-0.5">
                              <MessageCircle className="h-2.5 w-2.5" /> {story.commentCount || 0}
                            </span>
                          </div>
                          <button
                            onClick={(e) => {
                            ...
                            className="rounded-lg bg-red-500/80 p-1 text-white transition-colors hover:bg-red-600"
                        ...
                        <span className="rounded-full bg-black/55 px-1.5 py-0.5 text-[9px] text-white/80">
        ...
        {isOwnProfile ? (
          <section className="relative order-2 overflow-hidden rounded-2xl border border-purple-300/15 bg-[#172338] shadow-xl shadow-purple-950/30">
            ...
            <div className="relative border-b border-white/10 bg-gradient-to-l from-fuchsia-500/25 via-purple-500/15 to-transparent px-4 py-3">
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-fuchsia-400/70 to-purple-600/70 shadow-lg shadow-purple-950/30 ring-1 ring-white/25">
                    <User className="h-4 w-4 text-white" />
                  </span>
                  <div>
                    <h2 className="text-sm font-black tracking-wide text-white">معلوماتك</h2>
                    <p className="mt-0.5 text-[10px] text-white/55">حدّث بياناتك لتظهر بصورة أفضل</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="relative space-y-3 p-4">
              <div className="group">
                <label className="mb-1 flex items-center gap-2 text-[10px] font-bold text-white/65">
                ...
                <div className="relative">
                  <User className="pointer-events-none absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2 text-purple-300/60 transition-colors group-focus-within:text-fuchsia-300" />
                   <input value={name} onChange={e => setName(e.target.value)} placeholder="اسمك"
                    className="w-full rounded-xl border border-white/10 bg-slate-950/65 py-2.5 pl-3 pr-10 text-sm text-white shadow-inner shadow-black/20 placeholder:text-white/25 outline-none transition-all focus:border-fuchsia-400/60 focus:bg-slate-950/90 focus:ring-4 focus:ring-fuchsia-500/10"
                  />
                </div>
              </div>
                ...
                <div className="relative">
                  <Calendar className="pointer-events-none absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2 text-purple-300/60 transition-colors group-focus-within:text-fuchsia-300" />
                   <input type="number" value={age} onChange={e => setAge(Number(e.target.value))} min={13} max={100}
                    className="w-full rounded-xl border border-white/10 bg-slate-950/65 py-2.5 pl-3 pr-10 text-sm text-white shadow-inner shadow-black/20 outline-none transition-all focus:border-fuchsia-400/60 focus:bg-slate-950/90 focus:ring-4 focus:ring-fuchsia-500/10"
                  />
                </div>
              </div>
              ...
              <div className="group">
                ...
                <textarea value={bio} onChange={e => setBio(e.target.value)} placeholder="اكتب شيئاً عن نفسك..." rows={3}
                   className="w-full resize-none rounded-xl border border-white/10 bg-slate-950/65 px-3 py-2.5 text-sm text-white shadow-inner shadow-black/20 placeholder:text-white/25 outline-none transition-all focus:border-fuchsia-400/60 focus:bg-slate-950/90 focus:ring-4 focus:ring-fuchsia-500/10"
                />
              </div>
              ...
              <button
                ...
                disabled={saveProfile.isPending}
                 className="relative flex w-full items-center justify-center gap-2 overflow-hidden rounded-xl bg-gradient-to-l from-fuchsia-500 via-purple-600 to-violet-600 py-2.5 text-sm font-bold text-white shadow-lg shadow-purple-900/30 transition hover:brightness-110 disabled:opacity-60"
              >
          ...
          <section className="order-2 overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-b from-slate-800 to-slate-900 shadow-lg shadow-purple-950/20">
            <div className="border-b border-white/10 bg-gradient-to-l from-purple-500/15 via-fuchsia-500/10 to-transparent px-4 py-3">
              <h2 className="flex items-center gap-2 text-sm font-bold text-white">
                <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-purple-500/20 ring-1 ring-purple-300/20">
            ...
            {u?.bio && (
              <p className="mx-4 mb-4 rounded-xl border border-white/10 bg-slate-950/50 p-3 text-sm leading-relaxed text-white/70">
                {u.bio}
              </p>
            )}
          </section>
        ...
        {/* ── Credits & Stars ───────────────────────────────────────────── */}
        {isOwnProfile && <section className="order-5 overflow-hidden rounded-2xl border border-yellow-300/15 bg-gradient-to-b from-slate-800 to-slate-900 shadow-lg shadow-yellow-950/10">
          <div className="border-b border-white/10 bg-gradient-to-l from-yellow-500/15 via-orange-500/10 to-transparent px-4 py-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-yellow-400/15 ring-1 ring-yellow-300/25">
                  <Star className="h-4 w-4 fill-yellow-300 text-yellow-300" />
                </span>
                <div>
                  <h2 className="text-sm font-bold text-white">رصيد النجوم</h2>
                </div>
              </div>
              <div className="flex items-center gap-1.5 rounded-full border border-yellow-300/30 bg-yellow-400/10 px-2.5 py-1">
                <Star className="h-3 w-3 fill-yellow-300 text-yellow-300" />
                <span className="text-sm font-black text-yellow-200">{walletQuery.data?.wallet ?? 0}</span>
              </div>
            </div>
          </div>

          <div className="p-4">
            <div className="mb-3 rounded-xl border border-yellow-300/10 bg-yellow-300/[0.06] px-3 py-2">
              <p className="text-xs leading-5 text-white/60">استخدم نجومك لتفعيل رادار النجوم وإرسال هدايا افتراضية.</p>
            </div>

            <button onClick={() => setShowBuy(v => !v)}
              className="flex w-full items-center justify-center gap-2 rounded-xl border border-purple-400/40 bg-gradient-to-l from-purple-600/20 to-fuchsia-500/20 py-2.5 text-sm font-bold text-purple-100 transition hover:border-purple-300/70 hover:from-purple-600/30 hover:to-fuchsia-500/30"
            >
            ...
            {showBuy && (
              <div className="mt-3 space-y-2">
                <div className="flex items-center gap-2 rounded-xl border border-blue-400/20 bg-blue-500/10 px-3 py-2 text-xs font-medium text-blue-200">
                  <span>🔒</span>
                  <span>سيتم تفعيل الدفع قريباً</span>
                </div>
                {CREDIT_PACKAGES.map(pkg => (
                  <div key={pkg.credits} className={`relative flex items-center justify-between rounded-xl border p-3 transition ${
                    pkg.popular
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button disabled className="cursor-not-allowed rounded-lg bg-white/10 px-2.5 py-1 text-[10px] font-bold text-white/35">قريباً</button>
                    </div>
                  </div>
                ))}
              </div>
